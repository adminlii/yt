    ...
    public function setInvoice($invoice)
    {
    	...
    	if(!empty($invoice)){
            ...
            foreach($invoice as $k => $v){
                ...
                // 为传递币种,默认USD
                $v['invoice_currencycode'] = empty($this->_order['currencytype']) ? 'USD' : $this->_order['currencytype'];
                //抽离包装信息
                $v['packageid'] = $v['packId'];
                $v['packinfo']  = json_encode($package);
                //发件人增值税
                $v['invoice_shippertax'] = empty($this->_order['invoice_shippertax']) ? '' : $this->_order['invoice_shippertax'];
                //收件人增值税
                $v['invoice_consigneetax'] = empty($this->_order['invoice_consigneetax']) ? '' : $this->_order['invoice_consigneetax'];
                //申报总价
                $v['invoice_totalcharge_all'] = empty($this->_order['invoice_totalcharge_all']) ? 0 : $this->_order['invoice_totalcharge_all'];
                $invoice[$k] = $v;
            }
        }
...
//         print_r($sql);die;
    ...
    protected function _validate()
    {
        ...
        if(empty($this->_order['service_code'])){
        	$this->_err[] = Ec::Lang('服务类型不可为空');
        ...
        if(empty($this->_shipper)){
            ...
            }else if(!preg_match('/^[a-zA-Z\s\.&,]{1,25}$/',$this->_shipper['shipper_name'])){
        ...
        if($this->_consignee['mail_cargo_type'] == '4'){
	        if(empty($this->_invoice)||empty($this->_package)){
	            ...
	            foreach ($this->_package as $packk => $packv){
	            	$_packk = $packk+1;
	            	//检验
	            	if(! preg_match('/^[1-9](00|[0-9]?)$/', $packv['ITEMS']) || intval($packv['ITEMS']) <= 0){
	            		$this->_err[] = "(" . Ec::Lang('包裹信息') . $_packk . ")" . Ec::Lang('包裹件数必须为0-100的整数');
	            	}else{
	                    	$_totalpice += $packv['ITEMS'];
	                }
	                ...
	                if(empty($packv['LENGTH'])){
	                	...
	                	if(! preg_match('/^\d+(\.\d{1,2})?$/', $packv['LENGTH'])||$packv['LENGTH']<=0){
	                		$this->_err[] = "(" . Ec::Lang('包裹信息') . $_packk . ")" . Ec::Lang('包裹长度必须为数字,且小数最多为2位');
	                	}
	                }
	                
	                if(empty($packv['WIDTH'])){
	                	$this->_err[] = "(" . Ec::Lang('包裹信息') . $_packk . ")" . Ec::Lang('包裹宽度不可为空');
	                }else{
	                	if(! preg_match('/^\d+(\.\d{1,2})?$/', $packv['WIDTH'])||$packv['WIDTH']<=0){
	                		$this->_err[] = "(" . Ec::Lang('包裹信息') . $_packk . ")" . Ec::Lang('包裹宽度必须为数字,且小数最多为2位');
	                	}
	                }
	                if(empty($packv['HEIGHT'])){
	                	$this->_err[] = "(" . Ec::Lang('包裹信息') . $_packk . ")" . Ec::Lang('包裹高度不可为空');
	                }else{
	                	if(! preg_match('/^\d+(\.\d{1,2})?$/', $packv['HEIGHT'])||$packv['HEIGHT']<=0){
	                		$this->_err[] = "(" . Ec::Lang('包裹信息') . $_packk . ")" . Ec::Lang('包裹高度必须为数字,且小数最多为2位');
	                	}
	                }
	            }
	            //更新件数
	            if($_totalpice>100){
	            	$this->_err[] = "包裹件数不能超过100";
	            }else
	            ...
	            foreach($this->_invoice as $k => $invoice){ //Ec::showError("result:".print_r($this->_invoice,true)."\n", '_Ssssss_' . date('Y-m-d') . "_");
	                ...
	                if(!$invoice['invoice_quantity']){
	                    ...
	                    if(! preg_match('/^[1-9](00|[0-9]?)$/', $invoice['invoice_quantity']) || intval($invoice['invoice_quantity']) <= 0){
	                        $this->_err[] = "(" . Ec::Lang('申报信息') . $_k . ")" . Ec::Lang('申报数量必须为0-100的整数');
	                    }
	                }
...
//         print_r($this->_invoice);die;
        ...
        if($this->_order['invoice_print']){
        	//添加发票信息
        	$_label = array();
        	foreach ($this->_invoice as $labelk => $label ){
        		$_label['invoice_quantity'] = $label['invoice_quantity'];
        		$_label['invoice_note'] = $label['invoice_enname'];
        		$_label['invoice_shipcode'] = '';
        		$_label['invoice_unitcharge'] = $label['invoice_unitcharge'];
        		$_label['invoice_proplace'] = $label['invoice_proplace'];
        		$this->_label[] = $_label;
        	}
        }
        ...
        if(!empty($this->_invoice)){
            foreach($this->_invoice as $invoicek =>$row){
                ...
                $ivs = array(
                	...
                	'packageid' => empty($row['packageid'])?0:$row['packageid'],
                	'packinfo' => empty($row['packinfo'])?'':$row['packinfo'],
                	'invoice_proplace'=> empty($row['invoice_proplace'])?'':$row['invoice_proplace'],
                );
