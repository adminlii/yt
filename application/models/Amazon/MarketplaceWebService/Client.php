...
//     print_r($httpResponse['ResponseBody']);exit;
  ...
  private function reportAnyErrors($responseBody, $status, Exception $e =  null, $metadata)
  {
     ...
     *
     * @param $action -
     *            the MWS action to perform.
     * @param $parameters -
     *            the MWS parameters for the Action.
     * @param $dataHandle -
     *            A stream handle to either a feed to upload, or a report/feed submission result to download.
     * @param $contentMd5 -
     *            The Content-MD5 HTTP header value used for feed submissions.
     * @return array
     */
...
//         echo $httpResponse;exit;
        ...
        Ec::showError(print_r($parsedHeader,true),'___ama');
        
        require_once ('MarketplaceWebService/Model/ResponseHeaderMetadata.php');
        $responseHeaderMetadata = new MarketplaceWebService_Model_ResponseHeaderMetadata($parsedHeader['x-mws-request-id'], $parsedHeader['x-mws-response-context'], $parsedHeader['x-mws-timestamp']);
        ...
        Ec::showError($httpResponse,'__amazon_');
...
//       CURLOPT_VERBOSE => true,
  ...
  private function configureCurlOptions($action, array $converted, $streamHandle = null, $contentMd5 = null) {
    ...
    Amazon_Service::log($serviceUrl);
    $requestType = RequestType::getRequestType($action);
    
    //超时时间
    $curlOptions[CURLOPT_TIMEOUT]= 180;
    $curlOptions[CURLOPT_CONNECTTIMEOUT]= 180;
    ...
    if($requestType == RequestType::POST_UPLOAD){
            ...
            $fields = $this->getParametersAsString($converted[CONVERTED_PARAMETERS_KEY]);

            Ec::showError($fields,'__amazon_');
//             EC::showError($content,'_amazon');exit;
            Amazon_Service::log($fields);
  ...
  private function getFormattedTimestamp1($dateTime) {
    ...
    private function convertRequestReport($request) {
        ...
        if($request->isSetStartDate()){
            $parameters['StartDate'] = $this->getFormattedTimestamp1($request->getStartDate());
        }
        if($request->isSetEndDate()){
            $parameters['EndDate'] = $this->getFormattedTimestamp1($request->getEndDate());
        }
        if($request->isSetReportOptions()){
            $parameters['ReportOptions'] = $request->getReportOptions();
        }
        return array(
            CONVERTED_PARAMETERS_KEY => $parameters,
            CONVERTED_HEADERS_KEY => $this->defaultHeaders
        );
    }
