   ...
   private function collapseWhitespace($s) {
    ...
    private function _invoke(array $parameters)
    {
        ...
        try {
        	...
        	$parameters['Timestamp'] = new DateTime(date('Y-m-d H:i:s'), new DateTimeZone('UTC')); 
...
//     	print_r($parameters);exit;
        ...
        $ch = curl_init();
        $serviceUrl = $scheme . $url['host'] . $uri;
        Common_ApiProcess::log('请求地址:'.$serviceUrl); 
        //设置超时时间180s.
//         curl_setopt($ch, CURLOPT_TIMEOUT, 180);
        ...
        Common_ApiProcess::log('请求参数:'.$query); 
        curl_setopt($ch, CURLOPT_HEADER, true); 
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        
        if ($parameters['ProxyHost'] != null && $parameters['ProxyPort'] != -1)
        {
            curl_setopt($ch, CURLOPT_PROXY, $parameters['ProxyHost'] . ':' . $parameters['ProxyPort']);
        }
        ...
        //日志
        Ec::showError(print_r($response,true),'_amazon'); 
        list($other, $responseBody) = explode("\r\n\r\n", $response, 2);
        $other = preg_split("/\r\n|\n|\r/", $other);
		file_put_contents(APPLICATION_PATH.'/../data/log/'.$parameters['Action'].'.xml', $responseBody);
        $headers = array();
        ...
        Common_ApiProcess::log('响应头:'.print_r($headers,true));
    ...
    private function _convertGetOrder($request) {
        ...
        if ($request->isSetAmazonOrderId()) {
            ...
            foreach  ($amazonOrderIdgetOrderRequest->getId() as $idamazonOrderIdIndex => $idamazonOrderId) {
            	if(is_array($idamazonOrderId)){
            		foreach($idamazonOrderId as $index=>$id){
            			$parameters['AmazonOrderId' . '.' . 'Id' . '.'  . ($index + 1)] =  $id;            			
            		}            		
            	}else{
