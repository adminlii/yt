<?php
class Table_AfterShipCouriers
{
    protected $_table = null;

    public function __construct()
    {
        $this->_table = new DbTable_AfterShipCouriers();
    }
    ...
    {
        return new Table_AfterShipCouriers();
    }
    ...
    public function getByCondition($condition = array(), $type = '*', $pageSize = 0, $page = 1, $orderBy = "")
    {
        ...
        if(isset($condition["other_name"]) && $condition["other_name"] != ""){
            $select->where("other_name = ?",$condition["other_name"]);
        }
        if(isset($condition["web_url"]) && $condition["web_url"] != ""){
            $select->where("web_url = ?",$condition["web_url"]);
        }
        /*CONDITION_END*/
        if ('count(*)' == $type) {
