<?php
class Table_ProductLog
{
    protected $_table = null;

    public function __construct()
    {
        $this->_table = new DbTable_ProductLog();
    }
    ...
    public function getByCondition($condition = array(), $type = '*', $pageSize = 0, $page = 1, $orderBy = "")
    {
        ...
        if(isset($condition["pl_statu_pre"]) && $condition["pl_statu_pre"] != ""){
            $select->where("pl_statu_pre = ?",$condition["pl_statu_pre"]);
        }
        if(isset($condition["pl_statu_now"]) && $condition["pl_statu_now"] != ""){
            $select->where("pl_statu_now = ?",$condition["pl_statu_now"]);
        }
        if(isset($condition["pl_note"]) && $condition["pl_note"] != ""){
            $select->where("pl_note = ?",$condition["pl_note"]);
        }
        if(isset($condition["pl_add_time"]) && $condition["pl_add_time"] != ""){
            $select->where("pl_add_time = ?",$condition["pl_add_time"]);
        }
