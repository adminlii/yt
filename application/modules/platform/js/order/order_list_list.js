				...
				if(json.ask){
					...
					$.each(json.data.order_product, function(k, v) {
		        		...
		        		$.each(v.sub_product,function(sub_k,sub){
    			            ...
    			            html +='</div>';
		        			html += '<div class="warehouseSkuWrap"><{t}>对应品名<{/t}>:'+'<a  href="javascript:;" >'+sub.pcr_product_sku + '</a>'+'</div>';
...
$(function(){
	$("table").on("click","#code",function(){
		var code=$(this).text();
		leftMenu('order_list','<{t}>订单管理<{/t}>','/order/order-list/list?shipper_hawbcode='+code+'&code_type=refer');
	});
});
