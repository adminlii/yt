...
function getProduct() {
	...
	$.ajax({
		...
		success : function(json) {
			var html = '';
			html +="<option value='' class='ALL'><{t}>-select-<{/t}></option>";
		...
		$.ajax({
			...
			success : function(json) {
				...
				$.each(json.results,function(k,v){
					...
					var ref_id = v.ref_id;
					var msg = v.message;
					html+="<{t}>平台单号<{/t}>:"+ref_id+"<{t}>处理结果<{/t}>:"+msg;
...
//			getProduct();
		...
		buttons : [{
			...
			click : function() {
				...
				if($('#product_code').val()==''){
					//alertTip("<{t}>运输方式不可为空<{/t}>");
					//return;
				}
				if($('.shipper_account:checked').size()==0){
					alertTip("<{t}>发件人信息 不可为空<{/t}>");
					return;
				}
				this_.dialog('close');
				verify('D');
	...
	$('.holdBtn').live('click', function() {
		...
		jConfirm('<{t}>are_you_sure<{/t}>', '<{t}>将选择的订单暂存<{/t}>', function(r) {
    		if(r){
    			$.ajax({
    				type : "POST",
