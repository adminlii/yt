                ...
                if (isJson(json) && json.state == '1') {
                    ...
                    $.each(data, function (k, v) {
                        if (v.menu_id != menu) {
                            ...
                            html += '<label style="cursor: pointer;" ><input type="checkbox" onclick="checkAll(this)" ref="checkbox' + menu + '"><span class="module-title"  ref="' + menu + '" >&nbsp;&nbsp;' + v.menu + "</span></label>";
                            html += "</div>";
                            html += "<div class='actionBox action_" + menu + "'>";
                        }
                        html += "<div class='actionDiv'>";
                        if (ids[v.id]) {
                            html += "<label style='cursor: pointer;' ><input type='checkbox' value='" + v.id + "' name='actionId[" + v.id + "]' checked='checked' class='actionId checkbox" + menu + "' /><span class='checked'>" + v.title + "</span></label>";
                        } else {
                            html += "<label style='cursor: pointer;' ><input type='checkbox' value='" + v.id + "' name='actionId[" + v.id + "]' class='actionId checkbox" + menu + "' />" + v.title + "</label>";
                        }
                        html += "</div>";
                        if (k == (data.length - 1)) {
                            html += "</div>";
