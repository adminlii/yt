<?php
class Auth_SystemController extends Ec_Controller_Action
{
    public function preDispatch()
    {
        $this->tplDirectory = "auth/views/user/";
        $this->serviceClass = new Service_UserSystem();
    }
    ...
    public function listAction()
    {
        if ($this->_request->isPost()) {
            ...
            if ($count) {
                ...
                $rows = $this->serviceClass->getByCondition($condition,$showFields, $pageSize, $page, array('us_id desc'));
                $return['data'] = $rows;
                $return['state'] = 1;
                $return['message'] = "";
            }
    ...
    public function getByJsonAction()
    {
        ...
        if (!empty($paramId) && $rows = $this->serviceClass->getByField($paramId, 'us_id')) {
