<?php
class Auth_LevelController extends Ec_Controller_Action
{
    public function preDispatch()
    {
        $this->tplDirectory = "auth/views/user/";
        $this->serviceClass = new Service_UserPositionLevel();
    }
    ...
    public function listAction()
    {
        if ($this->_request->isPost()) {
            ...
            if ($count) {
                ...
                $rows = $this->serviceClass->getByCondition($condition,$showFields, $pageSize, $page, array('upl_id desc'));
                $return['data'] = $rows;
                $return['state'] = 1;
                $return['message'] = "";
            }
        ...
        echo Ec::renderTpl($this->tplDirectory . "position_level_index.tpl", 'layout');
    }
    ...
    public function getByJsonAction()
    {
        ...
        if (!empty($paramId) && $rows = $this->serviceClass->getByField($paramId, 'upl_id')) {
