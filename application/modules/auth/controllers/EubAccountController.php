<?php
class Auth_EubAccountController extends Ec_Controller_Action
{
    public function preDispatch()
    {
        $this->tplDirectory = "auth/views/";
        $this->serviceClass = new Service_EubAccount();
    }
    ...
    public function listAction()
    {
        if ($this->_request->isPost()) {
            ...
            if ($count) {
                ...
                $rows = $this->serviceClass->getByCondition($condition,$showFields, $pageSize, $page, array('ea_id desc'));
                $return['data'] = $rows;
                $return['state'] = 1;
                $return['message'] = "";
            }
        ...
        echo Ec::renderTpl($this->tplDirectory . "eub_account_index.tpl", 'layout');
    }
    ...
    public function getByJsonAction()
    {
        ...
        if (!empty($paramId) && $rows = $this->serviceClass->getByField($paramId, 'ea_id')) {
