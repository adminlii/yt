			...
			$.each(results,function(k,json){
				...
				if(json.ask){
					...
					$.each(json.data.order_product, function(k, v) {
			            ...
			            var give_up_title = parseInt(v.give_up)?'产品不发':'产品正常处理';
			            html +='<span class="unitPrice give_up_'+v.give_up+'" style="width:100px;float:right;" title="'+give_up_title+'">单价:';			            
...
//    		html += '<td style="word-wrap:break-word;">' + (val.order_desc == undefined ? '' : val.order_desc) + '</td>';
		html += '<p style="" class="buyerId ellipsis">买家: ' +  val.buyer_name +'</p>'; 
		//html += '<p style="" class="buyerMail ellipsis">买家Email: ' +val.buyer_mail+'</p>'; 
		html += '<p style="" class="userAccount ellipsis">卖家: <b>' +  user_account_arr_json[val.user_account].platform_user_name + '</b></p>'; 
...
//		} 
        ...
        /*
        html += ' <span class="viewIcon " ref_id="'+val.refrence_no_platform+'" style="float: right;cursor: pointer;" title="查看Ebay数据">查看Ebay数据</span>';
        
        html += ' <span class="completeIcon " ref_id="'+val.refrence_no_platform+'" style="float: right;cursor: pointer;" title="手动标记发货到Ebay">手动标记发货到Ebay</span>';
        
        html += ' <span class="loadLogIcon " ref_id="'+val.refrence_no_platform+'" style="float: right;cursor: pointer;" title="查看Load日志"></span>';

        html += ' <span class="logIcon " ref_id="'+val.refrence_no_platform+'" style="float: right;cursor: pointer;" title="查看操作日志"></span>';
        */
        ...
        if(val.sync_status==1){
            html += '<span class="iconshipamazon" style="cursor: pointer;float:left;" title="已标记发货,并同步到Amazon"></span>';
        }else if(val.sync_status==3 && val.order_type == 'sale' && val.create_type == 'api'){
            html += '<span class="iconshipamazonAgain" style="cursor: pointer;float:left;" title="更新了跟踪号等信息，等待同步中"></span>';
        }else if(val.sync_status==4){
            html += '<span class="iconshipamazonWait" style="cursor: pointer;float:left;" title="标记发货中,等待Amazon响应"></span>';
        }
        if(val.order_desc){
...
//			html += '<span class="iconshipamazon" style="cursor: pointer;float:left;" title="异常信息"></span>';
        ...
        html += '<div style="clear:both;width:100%;">';
        	html += 	'<div class="ellipsis" style="float:left;width:33%;text-align:left;">';
        	html += 		'服务等级:<b>' + ((val.shipment_service_level_category)?val.shipment_service_level_category:'无') + ((val.fulfillment_channel && val.fulfillment_channel =='AFN')?'&nbsp;(<span style="color:#1B9301;">FBA</span>)':'') +'</b>';
        	html += 	'</div>';
        	html += 	'<div class="ellipsis" style="float:left;width:66%;text-align:left;">';
        	if(val.refrence_no){
        		html += '客户参考号:<b>' + val.refrence_no +'</b>';
        	}else{
        		html += '&nbsp;';
        	}
        	html += 	'</div>'; 			
        ...
        html += '<div style="width:100%;" class="order_detail order_merge_'+val.is_merge+'" id="order_detail_'+val.order_id+'"><div style="text-align: center;margin-top:30px;"><img src="/images/base/loading.gif"/></div></div>'; 
...
//		}
        ...
        html += '<p class="ellipsis">国家:<b><span  id="country_'+val.order_id+'">' + (val.consignee_country?val.consignee_country:"") + '</span></b></p>';
        html += '<p class="ellipsis">平台配送:'+(val.shipping_method_platform?val.shipping_method_platform:'未分配')+" </p>";
