...
<script type="text/javascript">
<{include file='order/js/order/order_list_list_common.js'}>
<{include file='order/js/order/order_process_tag.js'}>
<{include file=$jsfile}>
</script>
<style>
<!--
			...
			<div class="Tab">
				<ul>
					...
					<li style='position: relative;' class='mainStatus'>
						...
						<a href="javascript:void (0)" class="statusTag <{if $k!=''}>statusTag<{$k}><{else}>statusTagAll<{/if}>" status='<{$k}>' abnormal_type='' process_again=''>
						...
						<dl class='sub_status_container'>
							<!-- 
							<{if isset($status.process_again)}> <{foreach from=$status.process_again item=process_againArr name=process_againArr key=paK}>
							<dt class='order_status_tag_parent<{$k}>'>
								<a style="display: block;" href="javascript:void (0)" title='<{$process_againArr}>' class='process_again' abnormal_type='' process_again='<{$paK}>' status='<{$k}>'><{$process_againArr}></a>
							</dt>
							<{/foreach}> <{/if}> <{if isset($status.abnormal_type)}> <{foreach from=$status.abnormal_type item=abnormal name=abnormal key=abK}>
							<dt>
								<a style="display: block;" href="javascript:void (0)" title='<{$abnormal}>' class='abnormal_type' abnormal_type='<{$abK}>' process_again='' status='<{$k}>'><{$abnormal}></a>
							</dt>
							<{/foreach}> <{/if}> <{if isset($status.user_tag)}> <{foreach from=$status.user_tag item=tag name=tag key=tk}>
							<dt>
								<a style="display: block;" href="javascript:void (0)" title='<{$tag.text}>' class='ot_id' ot_id='<{$tag.ot_id}>' abnormal_type='' process_again='' status='<{$k}>'><{$tag.text}></a>
							</dt>
							<{/foreach}> <{/if}> 
							-->
						</dl>
				...
				<input type="button" value="<{t}>单票录入<{/t}>" class=" baseBtn" style="margin-right: 12px; float: right;" onclick="leftMenu('52','<{t}>单票录入<{/t}>','/order/order/create?quick=52')">
			...
			<table width="100%" cellspacing="0" cellpadding="0" border="0" class="table-module">
				...
				<tbody id="table-module-list-data">
					...
					<tr class="table-module-b1">
    					...
    					<td class="ec-center"><{t}>客户单号<{/t}></td>
    					<td class="ec-center"><{t}>运输方式<{/t}></td>
    					<td class="ec-center"><{t}>目的国家<{/t}></td>
    					<td class="ec-center"><{t}>收件人<{/t}></td>
    					<td class="ec-center"><{t}>跟踪单号<{/t}></td>
    					<td class="ec-center"><{t}>创建时间<{/t}></td>
    					<td class="ec-center"><{t}>入仓时间<{/t}></td>
    					<td class="ec-center"><{t}>来源<{/t}></td>
    					<td class="ec-center"><{t}>状态<{/t}></td>
    					<td class="ec-center"><{t}>operation<{/t}></td>
    				</tr>
					 -->
				</tbody>
...
<div id='print_dialog' title='<{t}>打印<{/t}>' class='dialog_div' style='display: none;line-height:22px;'>
...
<div id='editinvoice_dialog' title='<{t}>编辑申报信息<{/t}>' class='dialog_div' style='display: none;line-height:22px;'>
    <form method="POST" id="invoiceForm" onsubmit='return false;' target='_blank'>
    	<table cellspacing="0" cellpadding="0" width="100%" border="0" class="table-module">
			<tbody>
				<tr class="table-module-title">
					<td width="150"><{t}>单号<{/t}></td>
					<td width="150"><{t}>英文品名<{/t}><span class='msg'>*</span></td>
					<td width="150"><{t}>中文品名<{/t}><span class='msg'>*</span></td>
					<td width="50"><{t}>数量<{/t}><span class='msg'>*</span></td>
					<td width="50"><{t}>单位<{/t}></td>
					<td width="50"><{t}>单价<{/t}>($)<span class='msg'>*</span></td>
