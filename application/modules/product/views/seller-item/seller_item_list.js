...
EZ.getListData = function (json) {
	...
	$.each(json.data, function (key, val) {
		...
		if(val.sell_type!='2'){
			...
			$.each(val.relation,function(kk,vv){
		...
		html += '</div>';
		html += "</td>";
		html += "<td >" + val.price_sell+' '+val.currency + "</td>";
		html += "<td >" + (val.sell_qty-val.sold_qty) +' / '+val.sold_qty+ "</td>";
		...
		if(val.sell_type!='2'){
			if(val.supplySet.account_supply_tag){
				...
				if(val.supplySet.supply_type==1){
					html+="<a class='orgNode supply_warehouse' onclick='supply_warehouse(\""+val.user_account+"\",\""+val.sku+"\",\""+val.supplySet.supply_warehouse+"\");'>查看可补货数量</a><br/>";
				}
			}else{
		...
		html += '<li class="not_first"><a href="javascript:;" class="supplyLogBtn" item_id="'+val.item_id+'" type="2" title="'+$.getMessage('replenishment_log')+'">'+$.getMessage('replenishment_log')+'<!--补货日志--></a></li>';
...
//		}
		...
		/*
		//if(val.item_status=='Active'){
		var clone = $('#varation_table_tpl').clone();
		if(val.sell_type=='2'){
			$('.varation_tr',clone).addClass(clazz);
			$.each(val.variation,function(k,v){
				var clazz01 = (k + 1) % 2 == 1 ? "table-module-b1" : "table-module-b2";
				var sub_clone = $('#varation_table_sub_tpl .attr_tr').clone();
				sub_clone.addClass(clazz01);
				$('.var_sku',sub_clone).html('<a sku="'+v.sku+'" class="inventoryBtn" href="javascript:;"  title="点击查看库存">'+v.sku+'</a>');
				$('.var_qty',sub_clone).text(v.qty);
				$('.var_sold_qty',sub_clone).text(v.qty_sold);
				$('.var_start_price',sub_clone).text(v.start_pice+' '+v.currency);
				var attr = '';
				if(v.attr){
					$.each(v.attr,function(kk,vv){
						attr+=vv.name+' : <b>'+vv.val+'</b>,';
				...
				$('.var_attr',sub_clone).html(attr);

				if(val.item_status=='Active'){
					$('.var_supply_qty input',sub_clone).attr('name','supply_qty['+val.item_id+'_'+v.variation_id+']');
					$('.var_supply_qty input',sub_clone).attr('value',v.supply_qty);
					$('.var_supply_qty',sub_clone).append('<span>'+v.supply_qty+'</span>');
				}else{
					//补货数量
					$('.var_supply_qty',sub_clone).text(v.supply_qty);

				}

				$('.var_op',sub_clone).html('<p><a href="javascript:;" class="addToBlackListBtn" sku="'+v.sku+'" account="'+val.user_account+'">加入补货黑名单</a></p>');
				$('.var_op',sub_clone).append('<p><a href="javascript:;" class="releaseBlackListBtn" sku="'+val.sku+'" account="'+val.user_account+'">解除补货黑名单</a></p>');
				$('.table-module-list-data',clone).append(sub_clone);
			});
			//alert($('tbody',clone).html());
			html +=$('tbody',clone).html();
		}else if(val.sell_type=='1'){
			
			$('.var_attr',clone).remove();
			$('.var_op',clone).remove();

			$('.varation_tr',clone).addClass(clazz);
			var clazz01 = "table-module-b1";
			var sub_clone = $('#varation_table_sub_tpl .attr_tr').clone();
			sub_clone.addClass(clazz01);
			$('.var_attr',sub_clone).remove();
			$('.var_op',sub_clone).remove();

			$('.var_sku',sub_clone).html('<a sku="'+val.sku+'" class="inventoryBtn" href="javascript:;" title="点击查看库存">'+val.sku+'</a>');
			$('.var_qty',sub_clone).text(val.sell_qty);
			$('.var_sold_qty',sub_clone).text(val.sold_qty);
			$('.var_start_price',sub_clone).text(val.price_sell+' '+val.currency);

			if(val.item_status=='Active'){
				$('.var_supply_qty input',sub_clone).attr('name','supply_qty['+val.item_id+']');
				$('.var_supply_qty input',sub_clone).attr('value',val.supply_qty);
				$('.var_supply_qty',sub_clone).append('<span>'+val.supply_qty+'</span>');

			}else{
				//补货数量
				$('.var_supply_qty',sub_clone).text(val.supply_qty);
...
$(function(){
	...
	/*
	$('.batchEditBtn').toggle(function(){
		$('#listForm').addClass('editForm');
		$('.batchEditSubmitBtn').show();
		$('.batchEditQtySetBtn').show();
		$(this).val('取消批量编辑');
	},function(){
		$('#listForm').removeClass('editForm');
		$('.batchEditSubmitBtn').hide();
		$('.batchEditQtySetBtn').hide();
		$(this).val('批量编辑');
	});*/
	...
	*/
	...
	$('.syncBtn').click(function(){
		...
		$.ajax({
			type: "post",
			dataType: "json",
			url: '/product/seller-item/sync-supply-qty',
			data: param,
			success: function (json) {
				var html = '';
				if(json.ask){
			...
			if(flag){
				if(val.sell_type=='2'){ 
					$.each(val.variation,function(k,v){
						...
						html+='<td>';
						html+="<select name='var["+v.variation_id+"][status]'  class=' status' default='"+v.supplySet.status+"'>";
					...
					html+='<td>';
					html+="<select name='pu["+val.item_id+"][supply_type]'  class='supply_type' default='"+val.supplySet.supply_type+"'>";
					<{foreach from=$supply_type_arr key=k name=ob item=ob}>
				    html+="<option value='<{$k}>'><{$ob}></option>";
					<{/foreach}>
					html+="</select>";
					html+="<select name='pu["+val.item_id+"][supply_warehouse]' class='warehouse_code' default='"+val.supplySet.supply_warehouse+"'>";
					<{foreach from=$warehouseArr key=k name=ob item=ob}>
				    html+="<option value='<{$ob.warehouse_code}>'><{$ob.warehouse_code}>[<{$ob.warehouse_desc}>]</option>";
					<{/foreach}>
					html+="</select>";
					html+="<input type='text' name='pu["+val.item_id+"][supply_qty]' class='quantity input_text' default='"+val.supplySet.supply_qty+"' placeholder='<{t}>supply_qty<{/t}>'  value='"+val.supplySet.supply_qty+"'/>";
					
					
					html+='</td>';

					html+='<td>';
					html+="<select name='pu["+val.item_id+"][status]'  class=' status' default='"+val.supplySet.status+"'>";
					<{foreach from=$status_arr key=k name=ob item=ob}>
				    html+="<option value='<{$k}>'><{$ob}></option>";
					<{/foreach}>
					html+="</select>";
