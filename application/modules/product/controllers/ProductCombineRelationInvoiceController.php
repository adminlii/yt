<?php
class Product_ProductCombineRelationInvoiceController extends Ec_Controller_Action
{
    public function preDispatch()
    {
        $this->tplDirectory = "product/views/product-combine-relation-invoice/";
...
//             print_r($data);exit;
            foreach($data as $k=>$v){//删除旧数据
                /* 
                $comRows = Service_ProductCombineRelationProcess::getRelation($v['product_sku'],$v['user_account']);
                foreach($comRows as $kk=>$vv){
                    Service_ProductCombineRelation::delete($vv['pcr_id'],'pcr_id');
                }
                */
                $sql = "select * from product_combine_relation where product_sku='{$v['product_sku']}' and user_account='{$v['user_account']}'";
                $rs = $db->fetchAll($sql);
                foreach($rs as $r){
                    Service_ProductCombineRelation::delete($r['pcr_id'], 'pcr_id');
                }
...
//             print_r($data);exit;
    ...
    public function uploadAction(){
        ...
        if($this->_request->isPost()){
            set_time_limit(0);
            $file = $_FILES['fileToUpload'];
            $process = new Process_SkuRelationUpload();
            $return = $process->importTransaction($file);
...
// 	    	print_r($sub_sku_arr);
	    	...
	    	//删除旧数据
	    	$db = Common_Common::getAdapter();
	    	$sql = "delete from product_combine_relation where product_sku='{$product_sku}' and user_account='{$user_account}'";
	    	$db->query($sql);
// 	    	print_r($sub_sku_arr);
// 	    	exit;
...
//         print_r($rows);exit;
    ...
    public function editRelationSingleAction(){
        $user_account = $this->_getParam('user_account','');
        $product_sku = $this->_getParam('product_sku');
        $company_code = $this->_getParam('company_code','');
        $company_code = Common_Company::getCompanyCode();
        if($product_sku){
            $relations = Service_ProductCombineRelationProcess::getRelation($product_sku,$user_account,$company_code);
            ...
            if($user_account){
                ...
                $pUser = array('user_account'=>'','platform_user_name'=>'所有账户');
            }
            
            //         print_r($relations);exit;
            $this->view->relations = $relations;
            $this->view->user_account = $user_account;
            $this->view->pUser = $pUser;
            $this->view->company_code = $company_code;
            $this->view->product_sku = $product_sku;
        }else{
            $con = array(
                'company_code' => Service_User::getCustomerCode()
            );
            $orderBy = array('platform','status');
            $pUser = Service_PlatformUser::getByCondition($con,'*',0,0,$orderBy);
            $this->view->pUsers = $pUser;
        }
       
        echo $this->view->render($this->tplDirectory . "edit_relation_single.tpl");        
    }
...
//         print_r($invoiceArr);exit;
        ...
        try{
            ...
            if(empty($invoiceArr)){
                throw new Exception('缺少申报信息');
            }
            //             print_r($relationRow);exit;
            $totalPrice = 0;
            foreach($invoiceArr as $k=>$v){
                if(empty($v['pcr_product_sku'])){
                    throw new Exception('品名代码不能为空');
                }
                $con = array(
                    'invoice_code' => $v['pcr_product_sku'],
                    'company_code' => Common_Company::getCompanyCode()
                ...
                $totalPrice+=$invoiceInfo['invoice_unitcharge']*$v['pcr_quantity'];
                $v['pcr_pu_price'] = $invoiceInfo['invoice_unitcharge'];
                $v['pcr_product_sku'] = strtoupper($v['pcr_product_sku']);
                $invoiceArr[$k] = $v;
            }
            foreach($invoiceArr as $k=>$v){
                $v['pcr_percent'] = round($v['pcr_pu_price']/$totalPrice*100,3);
            ...
            foreach($invoiceArr as $v){
                ...
                $v['user_account'] = $acc;
                $v['product_sku'] = strtoupper($productSku);
                $v['pcr_add_time'] = date('Y-m-d H:i:s');
                Service_ProductCombineRelation::add($v);
                $log_content[] = implode('*', array_keys($v)).'='.implode('*', array_values($v));
            }
