...
class Zend_Loader_Autoloader_Resource implements Zend_Loader_Autoloader_Interface
{
    ...
    public function initMyResourceTypes($modules=array())
    {
        $this->getBasePath();
        $array=array();
        foreach($modules as $m){
            $array[$m]=array('namespace'=>$m,'path'=>$m);
        }
        
        //配置亚马逊libs库的绝对路径
        $array['MarketplaceWebServiceOrders']=array(
        		'namespace'=>'MarketplaceWebServiceOrders','path'=>'Amazon/MarketplaceWebServiceOrders'
        );
        //配置亚马逊libs库的绝对路径
        $array['MarketplaceWebService']=array(
        		'namespace'=>'MarketplaceWebService','path'=>'Amazon/MarketplaceWebService'
        );
