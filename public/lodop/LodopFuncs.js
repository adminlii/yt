...
function getLodop(oOBJECT, oEMBED) {
    ...
    try {
        ...
        if (!!window.ActiveXObject || "ActiveXObject" in window) LODOP = oOBJECT;
        if ((LODOP == null) || (typeof(LODOP.VERSION) == "undefined")) {
            if (navigator.userAgent.indexOf('Firefox') >= 0)
                document.documentElement.innerHTML = strHtmFireFox + document.documentElement.innerHTML;
            if (navigator.userAgent.indexOf('Win64') >= 0) {
                if (!!window.ActiveXObject || "ActiveXObject" in window) document.write(strHtm64_Install); else
                    document.documentElement.innerHTML = strHtm64_Install + document.documentElement.innerHTML;
            } else {
                if (!!window.ActiveXObject || "ActiveXObject" in window) document.write(strHtmInstall); else
                    document.documentElement.innerHTML = strHtmInstall + document.documentElement.innerHTML;
            }
            return;
            //LODOP.SET_LICENSES(lodopCp, lodopKey, lodopToken, "");
            //return LODOP;
        } else if (LODOP.VERSION < "6.1.8.3" && false) {
            if (navigator.userAgent.indexOf('Win64') >= 0) {
                if (!!window.ActiveXObject || "ActiveXObject" in window) document.write(strHtm64_Update); else
                    document.documentElement.innerHTML = strHtm64_Update + document.documentElement.innerHTML;
            } else {
                if (!!window.ActiveXObject || "ActiveXObject" in window) document.write(strHtmUpdate); else
