...
function leftMenu(id, title, url) {
    var li_list = $(window.parent.document).find(".Menu_Main h3+ul>li");
    var finded = false;
    li_list.each(function (index, domEle){
    	var that = $(domEle);
    	var hreforginal = that.children("a").attr("hreforginal");
    	if(hreforginal.indexOf(id)>=0){
    		finded = true;
    		$(window.parent.document).find(".Menu_Main h3+ul>li>a").removeClass("curr");
    		that.children("a").addClass("curr");
    		//反选
    		$(window.parent.document).find(".Menu_Main h3").removeClass("on");
    		$(window.parent.document).find(".Menu_Main ul").hide();
    		that.prev("h3").addClass("on");
    		that.parent().show();
    		//刷新iframe
